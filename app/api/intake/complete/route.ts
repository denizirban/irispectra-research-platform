...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const images = await db<Array<{ id: string; laterality: string; quality_metrics: Record<string, unknown> }>>(`image_objects?submission_id=eq.${submissionId}&modality=eq.iris_still&select=id,laterality,quality_metrics`)
    ...
    const runs = await db<Array<{ id: string }>>(
      `analysis_runs?submission_id=eq.${submissionId}&model_family=eq.polar-regional-image-descriptors&model_version=eq.0.5&select=id&limit=1`,
    )
    const createdRun = !runs[0]
    if (createdRun) {
      await insert("analysis_runs", { submission_id: submissionId, status: "completed", completed_at: new Date().toISOString(), model_family: "polar-regional-image-descriptors", model_version: "0.5", pipeline_version: "2026-09-08-instance-mask", diagnostics: { scope: "non-diagnostic polar regional measurements plus local-contrast connected-component candidates", grid: "3 measurement zones x 12 clock sectors plus 6 atlas bands x 60 angular minutes", collarette: "image-derived estimate", instances: "experimental 2D photometric candidates" } })
    }
    ...
    return NextResponse.json({ success: true, reference: submissionId, notification, analysisState: "completed", acquisitionMetrics: images.map(image => ({ laterality: image.laterality, ...image.quality_metrics })) })
  } catch (error) {
