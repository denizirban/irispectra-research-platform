...
function phaseMedian(samples: Sample[], name: string) {
  const values = samples.filter(sample => sample.phase === name && sample.diameter_px !== null && sample.confidence >= .35).map(sample => sample.diameter_px!).sort((a, b) => a - b)
...
export function PupilProtocol() {
  ...
  const baselineMedian = phaseMedian(samples, "baseline")
  const lightMedian = phaseMedian(samples, "light")
  const recoveryMedian = phaseMedian(samples, "recovery")
  const amplitude = baselineMedian && lightMedian ? ((baselineMedian - lightMedian) / baselineMedian) * 100 : null
  const coverage = samples.length ? (usable.length / samples.length) * 100 : 0
  const meanConfidence = usable.length ? usable.reduce((sum, sample) => sum + sample.confidence, 0) / usable.length : 0
  const diameters = usable.map(sample => sample.diameter_px!)
  const low = diameters.length ? Math.min(...diameters) : 0
  const high = diameters.length ? Math.max(...diameters) : 1
  const range = Math.max(.01, high - low)
  const trace = usable.map(sample => `${Math.max(0, Math.min(100, sample.t_ms / 200))},${38 - ((sample.diameter_px! - low) / range) * 30}`).join(" ")
  return <div className="form-wrap">
    ...
    <section className="form-section"><div className="button-row"><button className="btn" type="button" onClick={enable} disabled={camera === "ready" || running}>Enable camera</button><button className="btn primary" type="button" onClick={run} disabled={camera !== "ready" || running}>{running ? "Protocol running…" : "Run 20-second pilot"}</button><button className="btn" type="button" onClick={download} disabled={!samples.length || running}>Export CSV</button></div>
      ...
      <div className="pupil-trace" aria-label="Pupil width proxy over the twenty second protocol">
        <header><div><span>LIVE TRACE · PIXEL PROXY</span><strong>{running ? phase.name.replace("_", " ") : samples.length ? "protocol captured" : "awaiting camera"}</strong></div><span>{elapsed ? `${(elapsed / 1000).toFixed(1)} s` : "20.0 s protocol"}</span></header>
        <svg viewBox="0 0 100 44" role="img" aria-label="Tracked pupil diameter proxy across protocol phases">
          <rect x="0" y="0" width="25" height="44" className="phase-dark" /><rect x="25" y="0" width="25" height="44" className="phase-light" /><rect x="50" y="0" width="25" height="44" className="phase-dark" /><rect x="75" y="0" width="25" height="44" className="phase-light" />
          {[8, 23, 38].map(y => <line key={y} x1="0" y1={y} x2="100" y2={y} />)}
          {trace && <polyline points={trace} />}
        </svg>
        <footer><span>BASELINE</span><span>LIGHT</span><span>RECOVERY</span><span>LIGHT REPEAT</span></footer>
      </div>
      <div className="pupil-metrics"><div><small>samples</small><strong>{samples.length}</strong></div><div><small>tracking coverage</small><strong>{samples.length ? `${coverage.toFixed(0)}%` : "—"}</strong></div><div><small>mean confidence</small><strong>{usable.length ? meanConfidence.toFixed(2) : "—"}</strong></div><div><small>median proxy</small><strong>{median ?? "—"} px</strong></div><div><small>first light change</small><strong>{amplitude === null ? "—" : `${amplitude.toFixed(1)}%`}</strong></div><div><small>recovery proxy</small><strong>{recoveryMedian ?? "—"} px</strong></div></div>
